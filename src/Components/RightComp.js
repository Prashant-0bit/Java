...
import robotArmImage from './RobotCoordinate.png';

const RightComp = () => {
  const [selectedIcon, setSelectedIcon] = useState(null);
  const [showMenu, setShowMenu] = useState(false);
  const menuRef = useRef(null);
  const [sidebarHeight, setSidebarHeight] = useState(window.innerHeight - 249); // Subtract the height of the navbar
  const [sidebarLabels, setSidebarLabels] = useState(['X', 'Y', 'Z', 'A', 'B', 'C']);
  const [isAxisSelected, setIsAxisSelected] = useState(false);

  const handleResize = () => {
    setSidebarHeight(window.innerHeight - 249); // Subtract the height of the navbar
  };
