...
const RightComp = () => {
  ...
  const [sidebarHeight, setSidebarHeight] = useState(window.innerHeight - 246); // Subtract the height of the navbar
  const [sidebarLabels, setSidebarLabels] = useState(['X', 'Y', 'Z', 'A', 'B', 'C']);
  const [isAxisSelected, setIsAxisSelected] = useState(false);

  const handleResize = () => {
    setSidebarHeight(window.innerHeight - 246); // Subtract the height of the navbar
  };
